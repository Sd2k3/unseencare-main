...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (elem: HTMLElement, e: MouseEvent) => {
    ...
    elems.forEach(elem => {
      elem.addEventListener("mouseenter", (e) => handleMouseEnter(elem as HTMLElement, e));
    });

    const elemContainer = document.querySelector("#elem-container");
    if (elemContainer) {
      elemContainer.addEventListener("mouseleave", handleMouseLeave);
      elemContainer.addEventListener("mousemove", handleMouseMove);
    }

    return () => {
      elems.forEach(elem => {
        elem.removeEventListener("mouseenter", (e) => handleMouseEnter(elem as HTMLElement, e));
      });
      if (elemContainer) {
        elemContainer.removeEventListener("mouseleave", handleMouseLeave);
        elemContainer.removeEventListener("mousemove", handleMouseMove);
      }
    };
  }, []);
